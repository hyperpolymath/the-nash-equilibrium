...
launcher-standard A2ML manifest, standards#960) plus the standing instruction to align
...
* *`.github/workflows/actions.lock` regenerated* with `gh actions-lock`
  v0.1.6 (built from source after the release-asset host proved unreachable),
  then hand-corrected for job-level reusable refs the tool cannot see (six
  standards callers at `da2c748a`, release.yml's SLSA generator, codeql's
  de-pinned SHA) per the `check-lock-sync.sh` fix procedure. `--verify-local`
  valid; all four lock-sync clauses green (Node port of the gawk script).
* *Push + PR done* — PR #119 (closes #117).
* *CI on PR #119 cannot RUN: actor gate (org-side).* Every run triggered by
  `arena-ai-coding-agent[bot]` ends `startup_failure` with zero jobs and the
  annotation `Actor is not allowed to trigger Actions workflows` — the GitHub
  App is blocked org-wide, not by this branch's files (base-lock bisect also
  failed all runs; a clean-tree control would fail identically). Documented in
  `hyperpolymath/wordpress-tools#96`; standards#968/#992 cover the adjacent
  lockfile semantics. Owner actions there: permit the App under org Settings →
  Actions → General, re-run as the owner, or merge as the owner. Main's own
  CI has shown the same signature for bot-triggered runs since 2026-09-25.
