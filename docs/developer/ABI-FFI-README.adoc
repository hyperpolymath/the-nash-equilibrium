= the-nash-equilibrium ABI/FFI Documentation
...
│  generated/abi/the-nash-equilibrium.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libthe-nash-equilibrium.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── the-nash-equilibrium.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── the-nash-equilibrium.h       # Generated from Idris2 ABI
│
...
----
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/the-nash-equilibrium.h
----
...
[source,c]
----
#include "the-nash-equilibrium.h"

int main() {
    void* handle = the-nash-equilibrium_init();
    if (!handle) return 1;

    int result = the-nash-equilibrium_process(handle, 42);
    if (result != 0) {
        const char* err = the-nash-equilibrium_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    the-nash-equilibrium_free(handle);
    return 0;
}
----
...
[source,bash]
----
gcc -o example example.c -lthe-nash-equilibrium -L./zig-out/lib
----
...
[source,idris]
----
import the-nash-equilibrium.ABI.Foreign
...
[source,rust]
----
#[link(name = "the-nash-equilibrium")]
extern "C" {
    fn the-nash-equilibrium_init() -> *mut std::ffi::c_void;
    fn the-nash-equilibrium_free(handle: *mut std::ffi::c_void);
    fn the-nash-equilibrium_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = the-nash-equilibrium_init();
        assert!(!handle.is_null());

        let result = the-nash-equilibrium_process(handle, 42);
        assert_eq!(result, 0);
...
[source,julia]
----
const libthe-nash-equilibrium = "libthe-nash-equilibrium"

function init()
    handle = ccall((:the-nash-equilibrium_init, libthe-nash-equilibrium), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:the-nash-equilibrium_process, libthe-nash-equilibrium), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:the-nash-equilibrium_free, libthe-nash-equilibrium), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   [source,bash]
   ----
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/the-nash-equilibrium.h
   ----
...
- link:https://idris2.readthedocs.io[Idris2 Documentation]
- link:https://ziglang.org/documentation/master/[Zig Documentation]
